...
def test_trailing_breakeven_stop() -> None:
    broker = PaperBroker()
    token = TokenRef(mint="BreakevenToken111111111111111111111111111")
    opened_at = datetime.now(timezone.utc) - timedelta(seconds=60)

    # Opened at 0.0001, pumped to 0.000135 (+35%), now dropped back to 0.0001005 (+0.5%)
    pos = Position(
        token=token,
        opened_at=opened_at,
        amount_token=1000.0,
        cost_basis_native=0.10,  # entry price = 0.0001
        last_price_native=0.0001005,  # <= entry + 1% floor
        peak_price_native=0.000135,   # >= entry + 30% gain
    )
    broker.account.positions[token.key] = pos
    ...
    fraction, reason = signals[0]
    assert fraction == 1.0
    assert "trailing breakeven stop" in reason
